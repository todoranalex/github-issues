...
export const addBookmark = (bookmark: Issue) => async (dispatch: React.Dispatch<Action>) => {
  dispatch({ type: 'add-bookmark' })
  await toggleBookmark(bookmark)
  dispatch({ type: 'add-bookmark-success', payload: { bookmark } })
}

export const removeBookmark = (bookmark: Issue) => async (dispatch: React.Dispatch<Action>) => {
